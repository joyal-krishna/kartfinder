    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      {!wishlists?.length ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {wishlists.map((w: any) => {
            const products = (w.products || []) as any[]
            const total = products.reduce((s: number, p: any) => s + p.current_price * p.quantity, 0)
            const platforms = [...new Set(products.map((p: any) => p.platform))]
            return (
              <Link key={w.id} href={`/dashboard/wishlists/${w.id}`} className="card p-5 hover:shadow-md transition-shadow cursor-pointer block">
                ...
                <div className="flex items-center justify-between text-sm">
                  ...
                  {total > 0 && <span className="font-semibold text-brand-600">Rs.{total.toLocaleString('en-IN', { maximumFractionDigits: 0 })}</span>}
                </div>
              </Link>
            )
          })}
